...
//####################################################################################################################//
  ...
  transpose2D((float*&)weight1, layerSize, imagePixels);
  transpose2D((float*&)weight2, layerSize, layerSize);
  transpose2D((float*&)weight3, outputSize, layerSize);
...
)
{
  ...
  for (size_t i = 0; i < neuronCount; i++)
  {
    ...
    for (size_t t = 0; t < inputSize; t++) {
      k+=input[t]*weight[i*inputSize+t];
    }
    output[i] = (k + bias[i]) > 0.0 ? (k + bias[i]) : 0.0 ;
    // output[i] = evalNeuron(inputSize, /*neuronCount,*/ input, weight+(i*inputSize), bias[i]/*, i*/);
  }
}
