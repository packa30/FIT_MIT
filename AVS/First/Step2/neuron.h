...
// #pragma omp declare simd simdlen(8) uniform(inputSize,input) linear(weight:784) aligned(weight)
// #pragma omp declare simd simdlen(8) uniform(inputSize,input) linear(weight:512) aligned(weight)
#pragma omp declare simd simdlen(8) uniform(inputSize,input) linear(weight) aligned(weight)
float evalNeuron(
  size_t inputSize,
  //size_t neuronCount,
  const float* input,
